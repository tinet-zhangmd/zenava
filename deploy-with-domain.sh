...
if [ -n "$DOMAIN" ]; then
    echo ""
    echo "🌐 绑定域名: $DOMAIN"
    if npx wrangler pages domain add "$DOMAIN" --project-name "$PROJECT_NAME" 2>/dev/null; then
        echo "✅ 域名绑定成功"
    else
        echo "⚠️  域名绑定失败，可能原因："
        echo "   - 域名已绑定"
        echo "   - 域名未在 Cloudflare 管理"
        echo "   - DNS 配置问题"
        echo ""
        echo "💡 提示：可以在 Cloudflare Dashboard 中手动绑定域名"
    fi
fi
...
if [ -n "$DOMAIN" ]; then
    ...
    echo "  🌐 中文首页: https://$DOMAIN/zh"
    echo "  🌐 AI Agents: https://$DOMAIN/products/ai-agents"
fi
echo "  🌐 Pages 域名: https://$PROJECT_NAME.pages.dev"
echo ""
echo "📝 后续操作:"
echo "  - 查看部署列表: npx wrangler pages deployment list --project-name $PROJECT_NAME"
echo "  - 查看域名列表: npx wrangler pages domain list --project-name $PROJECT_NAME"
